...
namespace MVCGrid.Web
{
    ...
    internal class MVCGridHtmlGenerator
    {
        ...
        internal static string GenerateClientDataTransferHtml(GridContext gridContext)
        {
            ...
            sb.AppendFormat("\"name\": \"{0}\"", HtmlUtility.JavaScriptStringEncode(gridContext.GridName));
            sb.Append(",");
            sb.AppendFormat("\"sortColumn\": \"{0}\"", HtmlUtility.JavaScriptStringEncode(gridContext.QueryOptions.SortColumnName));
            sb.Append(",");
        ...
        private static string GenerateClientJsonVisibility(GridContext gridContext)
        {
            ...
            foreach (var cv in gridContext.QueryOptions.ColumnVisibility)
            {
                ...
                sb.AppendFormat("\"{0}\": \"{1}\"", "headerText", HtmlUtility.JavaScriptStringEncode(gridColumn.HeaderText));
                sb.Append(",");
        ...
        private static string GenerateClientJsonAdditional(GridContext gridContext)
        {
            ...
            foreach (var aqon in gridContext.GridDefinition.AdditionalQueryOptionNames)
            {
                ...
                sb.AppendFormat("\"{0}\": \"{1}\"", aqon, HtmlUtility.JavaScriptStringEncode(val));
            }
            return sb.ToString();
        }
        ...
        private static string GenerateClientJsonFilter(GridContext gridContext)
        {
            ...
            foreach (var col in filterableColumns)
            {
                ...
                sb.AppendFormat("\"{0}\": \"{1}\"", col.ColumnName, HtmlUtility.JavaScriptStringEncode(val));
            }
            return sb.ToString();
        }

        internal static string GenerateBasePageHtml(string gridName, IMVCGridDefinition def, object pageParameters, string handlerPath)
        {
            ...
            if (renderLoadingDiv)
            {
                ...
                sbHtml.AppendFormat("&nbsp;&nbsp;&nbsp;<img src='{0}/ajaxloader.gif' alt='{1}' style='width: 15px; height: 15px;' />", handlerPath, def.ProcessingMessage);
        ...
        private static string GenerateJsonPageParameters(object pageParameters)
        {
            ...
            foreach (var col in pageParamsDict)
            {
                ...
                sb.AppendFormat("\"{0}\": \"{1}\"", col.Key, HtmlUtility.JavaScriptStringEncode(val));
            }
            return sb.ToString();
        }
