    ...
    public class SimpleTemplatingEngine : IMVCGridTemplatingEngine
    {
        ...
        private object EvaluateParameter(string name, Models.TemplateModel model)
        {
            ...
            else
            {
                ...
                {
                    case "model":
                        // Cast away 'dynamic' so this stays a static call (no Microsoft.CSharp
                        // runtime-binder dependency); ReflectPropertyValue reflects on object anyway.
                        val = ReflectPropertyValue((object)model.Item, suffix);
                        break;
                    case "row":
