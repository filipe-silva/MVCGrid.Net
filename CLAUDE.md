...
- **`IGridResponse`** replaces `System.Web.HttpResponse` in `IMVCGridRenderingEngine.PrepareResponse` (members: `Clear`, `ContentType`, `BufferOutput`, `AddHeader`). Adapter impl: `MvcGridResponse`. Custom rendering engines implement `PrepareResponse(IGridResponse)`.
