...
- **MVCGrid.Wasm/** — a **netstandard2.0** browser/WebAssembly host adapter (assembly `MVCGrid.Wasm`, `IsPackable=false`), references the core. The in-browser analogue of the ASP.NET Core adapter: `WasmUrlBuilder`/`WasmQueryStringReader`/`WasmGridResponse` implement the three abstractions with no server (URLs → static pages, query string parsed from the browser URL, response state held in memory), and `WasmGridRenderer` mirrors `MvcGridRenderer` (`GetBasePageHtml`/`RenderData`/`RenderExport`/`GetClientScript`) by running the core `GridEngine` in-process. Added to the core's `InternalsVisibleTo`. Proves the core runs anywhere — no `System.Web`, no ASP.NET Core, no server at all.
- **MVCGrid.WasmExample/** — a **.NET WebAssembly browser app** (`Microsoft.NET.Sdk.WebAssembly`, net10, `[JSExport]` interop; the runtime is left resident — do **not** call `dotnet.run()`, which would exit it) that hosts the demo grid fully client-side. It reuses `MVCGrid.AspNetCoreExample`'s `Person`/`PeopleRepository`/`GridConfig` verbatim (linked `Compile` items) and the **unchanged** `MVCGrid.js` — `wwwroot/main.js` boots .NET, injects the grid HTML, and installs a jQuery `ajaxTransport` that answers MVCGrid.js's one AJAX call from WASM (plus a capture-phase export→Blob-download interceptor). **`PublishTrimmed=false` is required** (engines are instantiated by string type name via `Activator`, which trimming strips). Publishes to static files (`dotnet publish` → `wwwroot`) hostable on GitHub Pages. Needs the `wasm-tools` workload (`dotnet workload install wasm-tools`).
- **MVCGridExample/** — a net48 MVC web app that is both the demo site and the de-facto integration test surface (assembly name `MVCGrid.Web`). Every feature has a demo grid registered in `App_Start/MVCGridConfig.cs` and a matching view under `Views/Demo/`. The `Person` demo data is **in-memory** (`Data/SampleDatabaseEntities.cs` — a disposable class exposing `People` as `IQueryable<Person>`, seeded with 200 deterministic rows); there is **no database dependency** (the old Entity Framework EDMX + LocalDB `.mdf` were removed). Other demo repos (`JobRepo`, `TestItemRepository`, `DocumentationRepository`) are likewise in-memory/CSV.
...
- **docs/** — a Jekyll site published to GitHub Pages (`https://filipe-silva.github.io/MVCGrid.Net`, `baseurl: /MVCGrid.Net`). Hand-written HTML, not generated from code. Links must be baseurl-aware (`{{ 'x.html' | relative_url }}`); grid sample images live in `docs/images/`. The rendered nav lives in **`_layouts/default.html`** (`_data/page.yml` + `_includes/navigation.html` are unused legacy). `docs/demo.html` embeds the live WASM demo. Deployment is via **GitHub Actions** (`.github/workflows/pages.yml`): it Jekyll-builds `docs/` and `dotnet publish`es `MVCGrid.WasmExample` into `_site/demo`, then deploys the combined artifact — so the interactive demo lives at `/MVCGrid.Net/demo/`. Requires repo Pages source = "GitHub Actions".
